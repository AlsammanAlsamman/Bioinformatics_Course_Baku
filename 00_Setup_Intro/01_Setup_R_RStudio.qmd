...
4.  **(For PDF Output) Install LaTeX:** Open RStudio, go to the Console panel, and type the following commands one by one, pressing Enter after each:
    ...
    # install.packages("tinytex") # Run only once if you dont have it
    # tinytex::install_tinytex()  # Run only once to install LaTeX distribution
    \`\`\`
...
packages_to_install <- c(
  ...
  "vcfR"        # Optional: For reading VCF genotype files (can be complex)
  # Add any other packages identified during course development
)
...
if(length(new_packages)) {
  ...
  print("All required packages seem to be installed already.")
}

# --------------------------------------------
\`\`\`

After installation, you load packages into your current R session using \`library(package_name)\` at the start of your scripts or Quarto documents where needed.
